...
async function Scheduler(bot) {
    ...
    this.setReminder = async function (userId, channel, message) {
        ...
        const reminder = parser.getMessageAndDateFromReminderString(message);
    ...
    this.snoozeReminders = async function (userId, channel, message) {
        ...
        await agenda.jobs({name: reminderJobName, 'data.userId': userId, nextRunAt: {$eq: null}}, async (err, jobs) => {
